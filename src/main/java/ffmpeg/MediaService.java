 ...
 */
@WebServlet(urlPatterns = "/MediaService")
...
//        MediaDao mediaDao = DaoFactory.getMediaDao();
        ...
        //if("/uploadFile".equals(path)){
            ...
            try {
//                Media media = new Media(title, descript, src, picture, uptime);
...
//                        }
                        ...
                        //获得保存文件的路径
                        String basePath = "D:\\NFS";//sctx.getRealPath("videos");
                        //获得文件名
                        ...
                        media.setUptime(System.currentTimeMillis()+"");//(DateTimeUtil.getYMDHMSFormat());

                        //转码

                        flag = ffmpeg.executeCodecs(ffmpegPath, uploadFile.getAbsolutePath(), codcFilePath, mediaPicPath);
                    }
                }
                if(flag){
                    //转码成功,向数据表中添加该视频信息
