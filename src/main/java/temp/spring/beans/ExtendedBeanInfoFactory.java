...
/**
 * {@link org.springframework.beans.BeanInfoFactory} implementation that evaluates whether bean classes have
 ...
 * {@link org.springframework.beans.BeanInfoFactory} types to take precedence.
 *
 * @author Chris Beams
 * @since 3.2
...
public class ExtendedBeanInfoFactory implements BeanInfoFactory, Ordered {
	...
	/**
	 * Return an {@link org.springframework.beans.ExtendedBeanInfo} for the given bean class, if applicable.
	 */
	@Override
	public BeanInfo getBeanInfo(Class<?> beanClass) throws IntrospectionException {
		return (supports(beanClass) ? new org.springframework.beans.ExtendedBeanInfo(Introspector.getBeanInfo(beanClass)) : null);
	}
	...
	private boolean supports(Class<?> beanClass) {
		for (Method method : beanClass.getMethods()) {
			if (org.springframework.beans.ExtendedBeanInfo.isCandidateWriteMethod(method)) {
				return true;
			}
		}
		return false;
