...
public abstract class PropertyAccessorUtils {
	...
	public static String getPropertyName(String propertyPath) {
		int separatorIndex = (propertyPath.endsWith(org.springframework.beans.PropertyAccessor.PROPERTY_KEY_SUFFIX) ?
				propertyPath.indexOf(org.springframework.beans.PropertyAccessor.PROPERTY_KEY_PREFIX_CHAR) : -1);
	...
	public static boolean isNestedOrIndexedProperty(String propertyPath) {
		...
		for (int i = 0; i < propertyPath.length(); i++) {
			...
			if (ch == org.springframework.beans.PropertyAccessor.NESTED_PROPERTY_SEPARATOR_CHAR ||
					ch == org.springframework.beans.PropertyAccessor.PROPERTY_KEY_PREFIX_CHAR) {
				return true;
			}
		}
		return false;
	...
	private static int getNestedPropertySeparatorIndex(String propertyPath, boolean last) {
		...
		while (last ? i >= 0 : i < length) {
			switch (propertyPath.charAt(i)) {
				case org.springframework.beans.PropertyAccessor.PROPERTY_KEY_PREFIX_CHAR:
				case org.springframework.beans.PropertyAccessor.PROPERTY_KEY_SUFFIX_CHAR:
					inKey = !inKey;
					break;
				case org.springframework.beans.PropertyAccessor.NESTED_PROPERTY_SEPARATOR_CHAR:
					if (!inKey) {
						return i;
					}
			}
	...
	public static boolean matchesProperty(String registeredPath, String propertyPath) {
		...
		if (registeredPath.charAt(propertyPath.length()) != org.springframework.beans.PropertyAccessor.PROPERTY_KEY_PREFIX_CHAR) {
			return false;
		}
		return (registeredPath.indexOf(org.springframework.beans.PropertyAccessor.PROPERTY_KEY_SUFFIX_CHAR, propertyPath.length() + 1) ==
	...
	public static String canonicalPropertyName(String propertyName) {
		...
		int searchIndex = 0;
		while (searchIndex != -1) {
			int keyStart = sb.indexOf(org.springframework.beans.PropertyAccessor.PROPERTY_KEY_PREFIX, searchIndex);
			searchIndex = -1;
			if (keyStart != -1) {
				int keyEnd = sb.indexOf(
						org.springframework.beans.PropertyAccessor.PROPERTY_KEY_SUFFIX, keyStart + org.springframework.beans.PropertyAccessor.PROPERTY_KEY_PREFIX.length());
				if (keyEnd != -1) {
					String key = sb.substring(keyStart + org.springframework.beans.PropertyAccessor.PROPERTY_KEY_PREFIX.length(), keyEnd);
					if ((key.startsWith("'") && key.endsWith("'")) || (key.startsWith("\"") && key.endsWith("\""))) {
						sb.delete(keyStart + 1, keyStart + 2);
						sb.delete(keyEnd - 2, keyEnd - 1);
						keyEnd = keyEnd - 2;
					}
					searchIndex = keyEnd + org.springframework.beans.PropertyAccessor.PROPERTY_KEY_SUFFIX.length();
				}
			}
		}
		return sb.toString();
