 ...
 *
 * <p>It's possible to cast a proxy obtained from this factory to {@link temp.spring.aop.Advised},
...
public class ProxyFactoryBean extends ProxyCreatorSupport
	 ...
	 * @see #setInterfaces
	 * @see temp.spring.aop.AbstractSingletonProxyFactoryBean#setProxyInterfaces
	 */
	 ...
	 */
	private synchronized void initializeAdvisorChain() throws org.springframework.aop.framework.AopConfigException, BeansException {
		...
		if (!ObjectUtils.isEmpty(this.interceptorNames)) {
			...
			if (this.interceptorNames[this.interceptorNames.length - 1].endsWith(GLOBAL_SUFFIX) &&
				...
				throw new org.springframework.aop.framework.AopConfigException("Target required after globals");
			}
			...
			for (String name : this.interceptorNames) {
				...
				if (name.endsWith(GLOBAL_SUFFIX)) {
					if (!(this.beanFactory instanceof ListableBeanFactory)) {
						throw new org.springframework.aop.framework.AopConfigException(
	...
	private Advisor namedBeanToAdvisor(Object next) {
		try {
			...
			throw new org.springframework.aop.framework.AopConfigException("Unknown advisor type " + next.getClass() +
