...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	public boolean containsKey(Object key) {
		Reference<K, V> reference = getReference(key, Restructure.WHEN_NECESSARY);
		Entry<K, V> entry = (reference != null ? reference.get() : null);
		return (entry != null && org.springframework.util.ObjectUtils.nullSafeEquals(entry.getKey(), key));
	}
	...
	public boolean remove(Object key, final Object value) {
		return doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER, TaskOption.SKIP_IF_EMPTY) {
			...
			protected Boolean execute(Reference<K, V> reference, Entry<K, V> entry) {
				if (entry != null && org.springframework.util.ObjectUtils.nullSafeEquals(entry.getValue(), value)) {
					reference.release();
					return true;
				}
				return false;
	...
	public boolean replace(K key, final V oldValue, final V newValue) {
		return doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.SKIP_IF_EMPTY) {
			@Override
			protected Boolean execute(Reference<K, V> reference, Entry<K, V> entry) {
				if (entry != null && org.springframework.util.ObjectUtils.nullSafeEquals(entry.getValue(), oldValue)) {
	...
	protected static final class Entry<K, V> implements Map.Entry<K, V> {
		...
		public final boolean equals(Object other) {
			...
			return (org.springframework.util.ObjectUtils.nullSafeEquals(getKey(), otherEntry.getKey()) &&
					org.springframework.util.ObjectUtils.nullSafeEquals(getValue(), otherEntry.getValue()));
		}

		@Override
		public final int hashCode() {
			return (org.springframework.util.ObjectUtils.nullSafeHashCode(this.key) ^ org.springframework.util.ObjectUtils.nullSafeHashCode(this.value));
		}
	}
	...
	private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public boolean contains(Object o) {
			if (o != null && o instanceof Map.Entry<?, ?>) {
				...
				Reference<K, V> reference = org.springframework.util.ConcurrentReferenceHashMap.this.getReference(entry.getKey(), Restructure.NEVER);
		...
		public boolean remove(Object o) {
			if (o instanceof Map.Entry<?, ?>) {
				Map.Entry<?, ?> entry = (Map.Entry<?, ?>) o;
				return org.springframework.util.ConcurrentReferenceHashMap.this.remove(entry.getKey(), entry.getValue());
			}
			return false;
		}
		...
		@Override
		public int size() {
			return org.springframework.util.ConcurrentReferenceHashMap.this.size();
		}

		@Override
		public void clear() {
			org.springframework.util.ConcurrentReferenceHashMap.this.clear();
		}
	}
	...
	private class EntryIterator implements Iterator<Map.Entry<K, V>> {
		...
		private void moveToNextSegment() {
			...
			if (this.segmentIndex < org.springframework.util.ConcurrentReferenceHashMap.this.segments.length) {
				this.references = org.springframework.util.ConcurrentReferenceHashMap.this.segments[this.segmentIndex].references;
				this.segmentIndex++;
			}
		}
		...
		public void remove() {
			...
			org.springframework.util.ConcurrentReferenceHashMap.this.remove(this.last.getKey());
		}
	}
	...
	protected class ReferenceManager {
		...
		public Reference<K, V> createReference(Entry<K, V> entry, int hash, Reference<K, V> next) {
			if (org.springframework.util.ConcurrentReferenceHashMap.this.referenceType == ReferenceType.WEAK) {
